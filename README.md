# 📻 復古畫素風卡帶隨身聽 (Retro Pixel Cassette Player)
...
* **3D 卡帶翻面動畫**：當 Side A 播完時，隨身聽會自動「彈起」並停止播放。使用者點選「翻面 (FLIP)」後，卡帶會彈出並在空中進行 **3D 旋轉翻面** 後裝回，換至 Side B 播放。
...
* **手動新增模式（無需帳戶）**：不需連線 Spotify，直接自選機身外殼顏色、貼紙、條紋背景與手寫字色，輸入名稱後一鍵新增，並能立刻在首頁卡帶架上拿取播放（預設搭載免授權 Lofi 音樂）。
