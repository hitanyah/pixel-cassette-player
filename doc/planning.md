# 📻 專案規劃文檔 (Project Planning Document)

本文件詳述「復古像素風卡帶隨身聽」專案的產品規劃、系統架構、元件劃分及狀態設計。

---

## 1. 產品願景與定位
打造一個融合**復古硬體操作樂趣**與**現代串流音樂API**的網頁互動體驗。藉由擬真像素風格的視覺與音效，降低串流應用的冰冷感，為使用者帶來實體卡帶的溫暖與把玩趣味。
...
* **`Walkman.tsx`**：隨身聽機身組件，整合液晶螢幕、按鍵控制項與 3D 卡帶翻轉的動畫容器。
* **`DisplayScreen.tsx`**：像素液晶顯示螢幕，負責繪製 Canvas 波形與數字時鐘。
* **`CassetteTape.tsx`**：物理卡帶渲染元件，根據進度以 SVG 齒輪進行旋轉、並增減左右軸磁帶繞線寬度。
* **`CassetteRack.tsx`**：卡帶收納架，可點擊以自動化流程裝載卡帶。
* **`SettingsPage.tsx`**：卡帶編輯後台，處理外觀自訂與 Spotify PKCE 授權。
...
3. 播放器運行時，會動態將 `sideTime` 映射到對應的曲目及該曲目的內部播放進度（Offset）：
...
stateDiagram-v2
    ...
    SideFinished --> EjectAndFlip : 取出卡帶/點擊 Flip (進行3D翻轉動畫)
